...
wx.miniProgram.getEnv((res) => {
 ...
 if(province=="广东省"){
  ...
  if(locationurl.indexOf("?tg=")!=-1||locationurl.indexOf("&amp;poptype=1&amp;popkey=")!=-1){
    ...
    textEle += '			<p class="tishi" id="wx_tishi">小程序里不能打开外部链接</p>';
    textEle += '			<p class="tishi" id="wx_tishi2" style="color:#009688">微信关注回复口令即可进入！</p>';
    textEle += '      <select id="mySelect" onchange="selectcode()" style="margin-left:74px; width:171px;"></select>';
    textEle += '			<img class="wxewm" id="wxewm" src="">';
    textEle += '			<p class="gzcopy" id="wx_gzcopy">识别二维码，回复口令快速进入！</p>';
    textEle += '            <button class="copy-btn" id="wx_copyid" data-clipboard-text="" onclick="copythemessage1()"><img src="https://u3.huatu.com/guangdong/tupian/gdhtim05.png" style="position: absolute;right: 212px;bottom: 126px;">点击获取口令</button>';
    textEle += '		</li>';
...
function wxtc(url){
  ...
  // document.getElementById('mySelect').innerHTML = '123';
...
function selectcode() {
  ...
  wxewm.attr('src', 'https://gd.huatu.com/zt/gdhttg/dsggfjewm/' + selectedValue + '.png');
  var code = wxewm.attr('src');
  // console.log(code)
  $('.wxewm').attr('src',code);	
}
