							...
							if(localStorage.getItem('gdht_wxlogin_type')){
								...
								if((xcxfunction!='true')&&(window.location.href.indexOf("/zt/") != -1)){
									...
									if(window.location.href.indexOf("isxcx=false") != -1){
										...
										if (ua1.match(/(iPhone|iPod|iPad);?/i)) { //区分iPhone设备
											//console.log('IOS');
											checkRedirect(url);
											//loginOpen();
											event.preventDefault();
	...
	function xcxforminiprom1(url) {
		...
		if(alink.indexOf('go/u3')!=-1 || alink.indexOf("https://u3.huatu.com/uploads/soft/") != -1){
			alink = copytheu3all();
		}
		...
		if(textcopy=='-1'){
			...
			setTimeout(function () {
				document.getElementById('tishi3').innerHTML = '<a style="margin-right: 27%;width: 80%;"><img src="https://ah.huatu.com/z/pic/img/goin.gif"  style="float: left;margin-left: 21%;" alt="">'+textcopy+'</a>';
			}, 1000);
			setTimeout(function () {
				document.getElementById('tishi3').innerHTML = '';
			}, 6000);
	...
	function xcxforminiprom2(url,text) {
		...
		if(textcopy=='-1'){
			...
			setTimeout(function () {
				document.getElementById('tishi3').innerHTML = '<a style="margin-right: 27%;width: 80%;"><img src="https://ah.huatu.com/z/pic/img/goin.gif"  style="float: left;margin-left: 21%;" alt="">'+textcopy+'已复制成功！</a>';
			}, 1000);
			setTimeout(function () {
				document.getElementById('tishi3').innerHTML = '';
			}, 6000);
	...
	if (ua.match(/MicroMessenger/i) == "micromessenger") {
		wx.miniProgram.getEnv((res) => {
			if (res.miniprogram) {
				...
				textEle += '            <button class="copy-btn" id="wx_copyid" data-clipboard-text="" onclick="copythemessage1()"><img src="https://u3.huatu.com/guangdong/tupian/gdhtim05.png" style="position: absolute;right: 212px;bottom: 126px;">点击获取口令</button>';
				textEle += '		</li>';
	...
	if(window.location.href.indexOf('https://m.gd.huatu.com/jsks/2023/0228/4180867.html?wximg=https://u3.huatu.com/uploads/gdht/all/2023/0228/1677578810.png&amp;poptype=1&amp;popkey=')!=-1){
								//微信浏览器,插入弹窗
				document.write('<link rel="stylesheet" type="text/css" href="https://gd.huatu.com/zt/css.css?v=1"></link>');
				var title = '相关入口';
				var textEle = '	<div class="xn-wx-bg hide">';
	...
	function wxtc(url){
		...
		document.getElementById('wx_copyid').innerHTML = '<img src="https://u3.huatu.com/guangdong/tupian/gdhtim05.png" style="position: absolute;right: 212px;bottom: 126px;">点击获取口令';
		...
		if(textcopy=='-1'){
			...
			setTimeout(function () {
				document.getElementById('wx_tishi3').innerHTML = '<a style="margin-right: 27%;width: 80%;"><img src="https://ah.huatu.com/z/pic/img/goin.gif"  style="float: left;margin-left: 21%;" alt="">'+textcopy+'</a>';
			}, 1000);
			setTimeout(function () {
				document.getElementById('wx_tishi3').innerHTML = '';
			}, 6000);
...
function checkRedirect(url) {
   ...
   // if (!scheme) {
	...
	$.ajax({
		...
		type: "post",
		async:false,
		data: { aid: id,url:phoneurl,act:'getschemeforphone',alink:alink,timerid:timerid,'htitle':htitle,'tg':tg,'wx':wxstr},
		dataType: 'json',
